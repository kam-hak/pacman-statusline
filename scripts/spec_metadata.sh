...
# Optional: thoughts system status for breadcrumb/orientation data.
THOUGHTS_STATUS=""
if [ -n "${REPO_ROOT}" ] && command -v humanlayer >/dev/null 2>&1; then
  THOUGHTS_STATUS=$(
    cd "$REPO_ROOT"
    humanlayer thoughts status 2>/dev/null | head -n 40
  )
fi
